...
class AppRequestUtilsTests(unittest.TestCase):
    def test_read_json_file_detail_roundtrip_and_invalid_payload(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ({"ok": True}, None), app_request_utils.read_json_file_detail(good)
            )
            self.assertEqual(
                (None, "invalid_payload"), app_request_utils.read_json_file_detail(bad)
            )

    def test_parse_results_limit_and_validate_mode(self) -> None:
        self.assertEqual(
            25,
            app_request_utils.parse_results_limit(
                "?limit=25", default_limit=20, max_limit=100
            ),
        )
        self.assertEqual(
            "edit",
            app_request_utils.validate_mode("EDIT", valid_modes={"auto", "edit"}),
        )
        with self.assertRaises(ValueError):
            app_request_utils.parse_results_limit(
                "?limit=0", default_limit=20, max_limit=100
            )
    ...
    def test_decode_data_url_image_accepts_valid_payload_and_rejects_invalid(
        self,
    ) -> None:
