...
class TestImageModePayloads(unittest.TestCase):
    ...
    def test_edit_mode_payload_structure(self):
        ...
        self.assertIsNotNone(
            payload.get("input_image_id"), "Edit must have input_image_id"
        )
    ...
    def test_inpaint_mode_payload_structure(self):
        ...
        self.assertIsNotNone(
            payload.get("input_image_id"), "Inpaint must have input_image_id"
        )
        self.assertIsNotNone(
            payload.get("mask_image_id"), "Inpaint must have mask_image_id"
        )
    ...
    def test_edit_vs_inpaint_differentiation(self):
        ...
        self.assertTrue(
            edit_use_edit_image, "Edit should result in use_edit_image=True"
        )
        self.assertFalse(
            inpaint_use_edit_image, "Inpaint should result in use_edit_image=False"
        )
    ...
    def test_create_mode_no_input_image(self):
        ...
        # Assertions
        self.assertFalse(
            payload.get("use_input_image"), "Create mode must not use input image"
        )
        self.assertFalse(
            payload.get("use_inpainting"), "Create mode must not use inpainting"
        )
        self.assertNotIn(
            "input_image_id", payload, "Create must not have input_image_id"
        )
...
class TestImageModeStateManagement(unittest.TestCase):
    ...
    def test_task_defaults_for_edit(self):
        ...
        self.assertTrue(
            state["useInputImageCheckbox"], "Edit must enable input image checkbox"
        )
        self.assertFalse(
            state["useInpaintingCheckbox"], "Edit must disable inpainting checkbox"
        )
        self.assertEqual(
            state["denoiseStrength"], 0.25, "Edit must use IMG2IMG denoise"
        )
    ...
    def test_task_defaults_for_inpaint(self):
        ...
        self.assertTrue(
            state["useInputImageCheckbox"], "Inpaint must enable input image checkbox"
        )
        self.assertTrue(
            state["useInpaintingCheckbox"], "Inpaint must enable inpainting checkbox"
        )
        self.assertEqual(
            state["denoiseStrength"], 0.58, "Inpaint must use INPAINT denoise"
        )
    ...
    def test_task_defaults_for_create(self):
        ...
        self.assertFalse(
            state["useInputImageCheckbox"], "Create must disable input image checkbox"
        )
        self.assertFalse(
            state["useInpaintingCheckbox"], "Create must disable inpainting checkbox"
        )
