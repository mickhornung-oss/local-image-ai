...
class ImageInputValidationTests(unittest.TestCase):
    def test_normalize_optional_negative_prompt_and_source_type(self) -> None:
        self.assertEqual(
            image_input_validation.normalize_optional_negative_prompt(" no blur ", max_length=20),
            ("no blur", None),
        )
        self.assertEqual(
            image_input_validation.normalize_optional_negative_prompt(123, max_length=20),
    ...
    def test_parse_multi_reference_slot_and_identity_role(self) -> None:
        self.assertEqual(
            image_input_validation.parse_optional_multi_reference_slot_index("2", max_slots=3),
            2,
        )
        self.assertIsNone(
            image_input_validation.parse_optional_multi_reference_slot_index("auto", max_slots=3)
        )
        with self.assertRaises(ValueError):
            image_input_validation.parse_required_multi_reference_slot_index("", max_slots=3)
        self.assertEqual(
            image_input_validation.parse_required_identity_transfer_role(
                "identity_head_reference",
                allowed_roles=frozenset({"identity_head_reference", "target_body_image"}),
            ),
            "identity_head_reference",
        )
    ...
    def test_inspect_image_upload_and_mask_normalization(self) -> None:
        ...
        normalized_payload, normalized_info = image_input_validation.normalize_mask_upload_payload(
            payload,
    ...
    def test_validate_browser_mask_payload_checks_dimensions_and_empty_mask(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(image_input_validation.UploadRequestError) as mismatch_exc:
                image_input_validation.validate_browser_mask_payload(
            ...
            with self.assertRaises(image_input_validation.UploadRequestError) as empty_exc:
                image_input_validation.validate_browser_mask_payload(
    ...
    def test_parse_multipart_helpers_validate_expected_fields(self) -> None:
        ...
        original_name, payload, slot_index = image_input_validation.parse_multipart_multi_reference_image(
            f"multipart/form-data; boundary={boundary}",
            multipart,
            slot_index_parser=lambda value: image_input_validation.parse_optional_multi_reference_slot_index(value, max_slots=3),
        )
