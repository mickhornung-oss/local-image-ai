...
class RealisticUserWorkflows(unittest.TestCase):
    ...
    def test_user_workflow_bild_anpassen(self):
        ...
        # Verify expectations
        self.assertTrue(ui_state["useInputImage_checked"], 
                       "Edit mode must enable input image checkbox")
        self.assertFalse(ui_state["useInpainting_checked"], 
                        "Edit mode must disable inpainting checkbox")
        self.assertEqual(payload["task_id"], "edit", 
                        "Payload must include task_id='edit'")
        self.assertTrue(payload["use_input_image"], 
                       "Payload must set use_input_image=True")
        self.assertFalse(payload["use_inpainting"], 
                        "Payload must set use_inpainting=False")
        self.assertIsNotNone(payload["input_image_id"], 
                            "Payload must include input image")
    ...
    def test_user_workflow_bereich_im_bild_aendern(self):
        ...
        self.assertTrue(ui_state["useInpainting_checked"], 
                       "Inpaint mode must enable inpainting checkbox")
        self.assertEqual(payload["task_id"], "inpaint", 
                        "Payload must include task_id='inpaint'")
        self.assertTrue(payload["use_input_image"], 
                       "Payload must set use_input_image=True")
        self.assertTrue(payload["use_inpainting"], 
                       "Payload must set use_inpainting=True")
        self.assertIsNotNone(payload["input_image_id"], 
                            "Payload must include input image")
        self.assertIsNotNone(payload["mask_image_id"], 
                            "Payload must include mask image")
    ...
    def test_backend_handles_payloads_correctly(self):
        ...
        self.assertTrue(edit_use_edit_image, "Backend must recognize use_edit_image=True for edit")
        ...
        self.assertFalse(inpaint_use_edit_image, "Backend must recognize use_edit_image=False for inpaint")
    ...
    def test_no_mode_ambiguity_after_fixes(self):
        ...
        self.assertEqual(len(flags_set), 3, "All modes must have unique flag combinations")
        ...
        for flags in [edit_flags, inpaint_flags, create_flags]:
            ...
            if flags == edit_flags:
                self.assertTrue(use_edit_image, "Edit mode must result in use_edit_image=True")
            elif flags == inpaint_flags:
                self.assertFalse(use_edit_image, "Inpaint mode must result in use_edit_image=False")
            elif flags == create_flags:
                self.assertFalse(use_edit_image, "Create mode must result in use_edit_image=False")
...
class BackendCorrectnessValidation(unittest.TestCase):
    ...
    def test_edit_generates_correct_tuning(self):
        ...
        self.assertIsNotNone(negative_prompt_suffix, "Edit should use preservation suffix")
    ...
    def test_inpaint_generates_correct_tuning(self):
        ...
        self.assertIsNotNone(negative_prompt_suffix, "Inpaint should use locality suffix")
