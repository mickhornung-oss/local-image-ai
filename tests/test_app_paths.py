...
class AppPathsTests(unittest.TestCase):
    def test_repo_relative_path_prefers_repo_relative(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                "data/file.txt", app_paths.repo_relative_path(target, repo_root=root)
            )
    ...
    def test_path_to_web_path_and_identity_transfer_path_to_web_path(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            web_path = app_paths.path_to_web_path(
                path, root=root, route_prefix="/files/"
            )
    ...
    def test_resolve_request_path_rejects_escape_and_resolves_valid(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            valid = app_paths.resolve_request_path(
                "/files/a/b.png", route_prefix="/files/", root=root
            )
            invalid = app_paths.resolve_request_path(
                "/files/../b.png", route_prefix="/files/", root=root
            )

            self.assertEqual((root / "a" / "b.png").resolve(), valid)
            self.assertIsNone(invalid)

    def test_resolve_identity_transfer_role_request_path_and_reset_helpers(
        self,
    ) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
