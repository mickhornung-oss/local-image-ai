...
class TestImageModeE2EIntegration(unittest.TestCase):
    ...
    def test_edit_mode_complete_workflow(self):
        ...
        self.assertTrue(
            prepared["use_edit_image"], "Backend must infer use_edit_image=True"
        )
    ...
    def test_inpaint_mode_complete_workflow(self):
        ...
        self.assertFalse(
            prepared["use_edit_image"], "Backend must infer use_edit_image=False"
        )
        ...
        self.assertIsNotNone(
            prepared["input_image_id"], "Inpaint must have input image"
        )
    ...
    def test_edit_vs_inpaint_payload_differentiation(self):
        ...
        # Key differentiation
        self.assertTrue(
            edit_prepared["use_edit_image"],
            "Edit payload must result in use_edit_image=True",
        )
        self.assertFalse(
            inpaint_prepared["use_edit_image"],
            "Inpaint payload must result in use_edit_image=False",
        )
    ...
    def test_task_id_preserved_through_workflow(self):
        ...
        for task in ["create", "edit", "inpaint"]:
            ...
            self.assertEqual(
                prepared["task_id"], task, f"Task ID {task} must be preserved"
            )
...
class TestUIStateConsistency(unittest.TestCase):
    ...
    def test_mode_switch_affects_checkbox_state(self):
        ...
        self.assertTrue(
            state["useInputImage"], "Switching to edit must enable useInputImage"
        )
        self.assertFalse(
            state["useInpainting"], "Switching to edit must disable useInpainting"
        )
        ...
        self.assertTrue(
            state["useInputImage"], "Switching to inpaint must enable useInputImage"
        )
        self.assertTrue(
            state["useInpainting"], "Switching to inpaint must enable useInpainting"
        )
