...
class IdentityGenerateFlowTests(unittest.TestCase):
    def test_coerce_payload_and_prompt_checkpoint_normalization(self) -> None:
        ...
        normalized, normalize_error = identity_generate_flow.normalize_identity_prompt_and_checkpoint(
    ...
    def test_prepare_identity_reference_request_resolves_reference_and_errors(self) -> None:
        ...
        prepared, error = identity_generate_flow.prepare_identity_reference_request(
            ...
            resolve_reference_image=lambda value: (_ for _ in ()).throw(ValueError("missing_reference_image")),
        )
        self.assertIsNone(prepared)
        self.assertEqual(error["http_status"], HTTPStatus.BAD_REQUEST)
        self.assertEqual(error["blocker"], "missing_reference_image")

    def test_prepare_identity_research_request_adds_provider_and_negative_prompt(self) -> None:
        prepared, error = identity_generate_flow.prepare_identity_research_request(
            {"prompt": "Prompt", "provider": "  REPLICATE  ", "negative_prompt": " no blur "},
    ...
    def test_runtime_and_system_preflight_failures_map_consistently(self) -> None:
        runtime_error = identity_generate_flow.build_runtime_preflight_failure(
            {"ok": False, "error_type": "api_error", "blocker": "identity_transfer_unavailable"},
    ...
    def test_resolve_multi_reference_adapter_state_prefers_runtime_adapter(self) -> None:
