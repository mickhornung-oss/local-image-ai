...
if "requests" not in sys.modules:
    ...
    def _request(*args, **kwargs):
        raise AssertionError("requests.request should not be used in app_server integration tests")
...
class DummyHandler:
    def __init__(self, *, path: str = "/", json_body=None, body: bytes = b"", headers: dict | None = None, server=None) -> None:
        self.path = path
...
class AppServerIntegrationTests(unittest.TestCase):
    def test_resolve_default_text_model_profile_id_stays_on_standard(self) -> None:
        with patch.object(
            ...
            return_value={"current_profile_id": app_server.TEXT_MODEL_PROFILE_STRONG_WRITING},
        ):
            self.assertEqual(
    ...
    def test_handle_text_chat_create_returns_overview_payload(self) -> None:
        ...
        with patch.object(app_server.chat_requests, "coerce_optional_text_chat_payload", return_value=({"title": "Idee"}, None)), \
             patch.object(app_server.chat_requests, "normalize_create_text_chat_title", return_value=("Idee", None)), \
             patch.object(app_server, "create_text_chat_in_first_empty_slot", return_value={"slot_index": 0}), \
             patch.object(app_server, "build_text_chat_overview_payload", return_value={"status": "ok", "slots": []}):
    ...
    def test_handle_input_image_upload_maps_success_response(self) -> None:
        ...
        with patch.object(app_server.image_input_validation, "validate_multipart_content_type"), \
             patch.object(app_server, "parse_multipart_image", return_value=("input.png", b"payload", "file")), \
             patch.object(app_server, "store_uploaded_image", return_value={"image_id": "input-1", "source_type": "file"}), \
             patch.object(app_server, "build_upload_success_response", return_value={"status": "ok", "ok": True, "image_id": "input-1"}):
            app_server.AppRequestHandler.handle_input_image_upload(handler)

        self.assertEqual([(HTTPStatus.OK, {"status": "ok", "ok": True, "image_id": "input-1"})], handler.sent)
    ...
    def test_handle_result_delete_returns_success_payload(self) -> None:
        ...
        with patch.object(app_server, "delete_stored_result", return_value={"result_id": "result-1", "deleted": True}), \
             patch.object(app_server.result_output, "build_result_delete_success_response", return_value={"status": "ok", "ok": True, "result_id": "result-1", "deleted": True}):
            app_server.AppRequestHandler.handle_result_delete(handler)

        self.assertEqual([(HTTPStatus.OK, {"status": "ok", "ok": True, "result_id": "result-1", "deleted": True})], handler.sent)
    ...
    def test_do_post_generate_routes_through_general_flow_and_endpoint_flow(self) -> None:
        ...
        with patch.object(
            ...
            return_value=(
             ...
             patch.object(app_server.general_generate_flow, "build_general_generate_system_failure", return_value=None), \
             patch.object(
                 app_server.general_generate_flow,
             ...
             patch.object(app_server.generate_endpoint_flow, "try_begin_generate_render", return_value=None), \
             patch.object(app_server.generate_endpoint_flow, "execute_generate_endpoint", return_value=(HTTPStatus.OK, {"status": "ok", "result_id": "result-1"})):
            app_server.AppRequestHandler.do_POST(handler)

        self.assertEqual([(HTTPStatus.OK, {"status": "ok", "result_id": "result-1"})], handler.sent)
    ...
    def test_do_post_generate_returns_request_error_from_general_flow(self) -> None:
        ...
        with patch.object(
            app_server.general_generate_flow,
            "prepare_general_generate_request",
            return_value=(None, {"http_status": HTTPStatus.BAD_REQUEST, "error_type": "invalid_request", "blocker": "invalid_json"}),
        ):
            app_server.AppRequestHandler.do_POST(handler)
